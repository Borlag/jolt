...
# Mock all streamlit functions used
st.header = MagicMock()
st.info = MagicMock()
st.warning = MagicMock()
st.error = MagicMock()
st.multiselect = MagicMock(return_value=["Model A (ID: 12345678)", "Model B (ID: 87654321)"])
st.selectbox = MagicMock(side_effect=["Model A", "All Elements", "P1 - Row 1"]) # Ref Model, Element, Node Inspector
st.columns = MagicMock(return_value=[MagicMock(), MagicMock(), MagicMock()])
st.checkbox = MagicMock(return_value=True)
st.color_picker = MagicMock(return_value="#123456") # Mock color picker
st.slider = MagicMock(return_value=5.0)
st.markdown = MagicMock()
st.subheader = MagicMock()
st.dataframe = MagicMock()
st.table = MagicMock() # New for Node Inspector
