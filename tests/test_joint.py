...
def test_figure76_solution_matches_expected_response():
    ...
    assert max(abs(u) for u in solution.displacements) == pytest.approx(0.005565346212745123)
    ...
    assert reactions[("Tripler", 3)] == pytest.approx(-534.6723957484926)
    assert reactions[("Doubler", 6)] == pytest.approx(-465.3276042515063)
    ...
    assert row4["Doubler"] == pytest.approx((-84.44968706772897, -130.59928565917747), rel=1e-12)
    assert row4["Skin"] == pytest.approx((84.44968706772914, -250.27863152459992), rel=1e-12)

    tripler_left = next(item for item in solution.nodes_as_dicts() if item["Plate"] == "Tripler" and item["local_node"] == 0)
    assert tripler_left["u [in]"] == pytest.approx(0.0008051317229861027)
...
def test_figure76_beam_idealized_solution_matches_expected_response():
    ...
    assert len(solution.displacements) == 17
    assert max(abs(u) for u in solution.displacements) == pytest.approx(0.004359055853362962)
    ...
    reactions = {(item["Plate"], item["Global node"]): item["Reaction [lb]"] for item in solution.reactions_as_dicts()}
    assert reactions[("Tripler", 3)] == pytest.approx(-156.7706886876836)
    assert reactions[("Doubler", 6)] == pytest.approx(-843.2293113123158)
    ...
    assert row4["Doubler"] == pytest.approx((-82.21042304831602, 24.762020960237336), rel=1e-12)
    assert row4["Skin"] == pytest.approx((82.21042304831604, 132.00866772744655), rel=1e-12)

    tripler_left = next(item for item in solution.nodes_as_dicts() if item["Plate"] == "Tripler" and item["local_node"] == 0)
    assert tripler_left["u [in]"] == pytest.approx(0.00019391590287822367)
